...
/*!
    @brief  Clear contents of display buffer in selected area (set specifc all pixels to off).
    @return None (void).
    @note   Changes buffer contents only, no immediate effect on display.
            Follow up with a call to display(), or with other graphics
            commands as needed by one's own application.
*/
void Adafruit_SSD1306::clearDisplayArea(int x, int y, int width, int height)
{
